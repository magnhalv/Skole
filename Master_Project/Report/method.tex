...
We have made extensive use of Taiga Nomi's C++ framework for neural networks, available at \cite{SoftwareGithub}, in our project. The framework was used in order to train the parameters of our network, for measuring the efficency of a pure software implementation, and as a basis for the implementation that uses the hardware accelerator. The framework treats each layer in the network as a seperate module, which makes it easy to swap diffrent implementations of a layer. This simplified the process of integrating the hardware accelerator into the network.
...
In its current form, the accelerator is only able to compute one feature map at a time. Thus for C1/S2 the accelerator has to be used six times, and for C3/S4, sixteen times. 
...
A driver for the hardware accelerator was written in order to allow of easy communcation between it and the software. The hardware is memory mapped, thus the driver can easily configure and transfer data to the accelerator by writing to its memory addresses. The only configurations that are required is which layer is being computed, and starting the accelerator when data has been transfered to its buffer. 

For data transfer the driver uses a \textit{direct memory access controller} (DMA) IP from Xilinx. The DMA is configured to transfer the weights and image(s) to the accelerator's input buffer, and extract the data from the output buffer. Since the output buffer is a FIFO, the DMA is able to extract each output value as they are produced, instead of waiting for the accelerator to finish and then transfer all the output data. 
