...
\begin{figure}[h!]
  ...
  \caption{An example CNN, the LeNet-5 \cite{LeCun1998}. It consists of two layers of convolution followed by subsampling, which are connected to a fully connected ANN with 10 output classes.}
  \label{fig_cnn}
\end{figure}
...
The convolution layer performs the feature extraction by applying a filter on the whole image, and puts the result in a corresponding feature map. E.g. if the filter extracts vertical edges, only the vertical edges from the original image would remain in the resulting feature map. Thus different features can be extracted by having several feature maps with different filters. 

This is done by performing a \textit{convolution operation} on the image, using a kernel which acts like a filter. The kernel is a 2D matrix which contains a set of weights. Depending on values of the weights, convoluting the image with the kernel will have wide range of effects, e.g. sharpening, bluring, edge detection. By training our network we can configure the weight values to extract that features we need in order to recognize our desired classes.  
